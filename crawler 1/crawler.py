...
import time
from vulnerability_detectors import VulnerabilityAnalyzer
...
class NetworkTrafficCapturer:
    ...
    def __init__(self, page, analyzer=None):
        """
        Args:
            page: Playwright Page 객체
            analyzer: VulnerabilityAnalyzer 인스턴스 (옵션)
        """
        self.page = page
        ...
        self.analyzer = analyzer      # 취약점 분석기
    ...
    def _extract_endpoint(self, request, response):
        ...
        # 취약점 분석 실행
        if self.analyzer:
            endpoint = self.analyzer.analyze_endpoint(endpoint)
    ...
    def trigger_interactive_elements(self, max_clicks=10):
        ...
        for i, form in enumerate(forms):
            ...
            try:
                ...
                print(f"    [FORM] 폼 제출 시도: {form_method} {form_action}")
                ...
                for inp in inputs:
                    try:
                        ...
                        if any(keyword in input_name.lower() for keyword in dangerous_keywords):
                            print(f"      [SKIP]  위험한 필드 스킵: {input_name}")
                            continue
...
class AutomatedDynamicCrawler:
            ...
            for inp in form['inputs']:
                if inp['name']:
                    param = {
                        ...
                        'example_value': inp['value'],
                        'required': inp['required']
                    }
                    endpoint['parameters'].append(param)

            # 취약점 분석 실행
            endpoint = self.vulnerability_analyzer.analyze_endpoint(endpoint)
    ...
    def export_to_json(self, filename="integrated_crawl_results.json"):
        ...
        # Fuzzing Targets 생성
        fuzzing_targets = self.vulnerability_analyzer.generate_fuzzing_targets(all_endpoints)

        # SSRF/SSTI 후보 개수 집계
        ssrf_candidates = len([t for t in fuzzing_targets if t['vulnerability_type'] == 'SSRF'])
        ssti_candidates = len([t for t in fuzzing_targets if t['vulnerability_type'] == 'SSTI'])
        ...
        data = {
            ...
            'statistics': {
                ...
                'ssti_candidates': ssti_candidates,
                'total_vulnerability_targets': len(fuzzing_targets)
            },
        ...
        print(f"\n[+] 취약점 분석 결과:")
        print(f"    - SSRF 후보: {ssrf_candidates}개")
        print(f"    - SSTI 후보: {ssti_candidates}개")
        print(f"    - 총 Fuzzing Targets: {len(fuzzing_targets)}개")
