"""
동적 웹 크롤러 - Playwright 기반
블랙박스 테스트를 위한 동적 웹 애플리케이션 크롤링 및 API 엔드포인트 자동 추출
"""
...
class NetworkTrafficCapturer:
    ...
    def __init__(self, page):
        ...
        self.page_response_headers = {}  # 메인 페이지 응답 헤더 저장
    ...
    def get_page_response_headers(self):
        """메인 페이지 응답 헤더 반환 (보안 분석용)"""
    ...
    def clear_data(self):
        ...
        self.page_response_headers.clear()
...
class DynamicContentAnalyzer:
    ...
    def submit_forms_with_test_data(self, infer_value_func=None):
        """
        발견한 폼들에 추론된 값을 채워서 실제로 제출

        Args:
            infer_value_func: 파라미터 값 추론 함수 (AutomatedDynamicCrawler.infer_parameter_value)
        ...
        # 위험한 폼 필터링 키워드
        dangerous_actions = ['delete', 'remove', 'logout', 'reset', 'drop']

        # 현재 URL 저장
        original_url = self.page.url

        # 폼이 없으면 먼저 추출
        if not self.dynamic_forms:
            self.extract_dynamic_forms()

        for form_idx, form in enumerate(self.dynamic_forms):
            try:
                form_action = form['action'].lower()

                # 위험한 폼 스킵
                if any(keyword in form_action for keyword in dangerous_actions):
                ...
                print(f"  [{form_idx + 1}] 폼 제출 시도: {form['method'].upper()} {form['action']}")

                # 각 필드에 값 채우기
                filled_fields = []
                for inp in form['inputs']:
                    # name이 없거나 submit/button 타입은 스킵
                    if not inp['name'] or inp['type'] in ['submit', 'button', 'reset']:
                        continue

                    # 값 추론
                    if infer_value_func:
                        test_value = infer_value_func(
                            param_name=inp['name'],
                            param_type=inp['type'],
                        ...
                        # 기본값 사용
                        test_value = inp.get('value') or inp.get('placeholder') or 'test'
                    ...
                    try:
                        ...
                        # select 요소인 경우
                        if inp['type'] == 'select-one' or inp['type'] == 'select-multiple':
                            # 첫 번째 옵션 선택
                            self.page.select_option(selector, index=0, timeout=2000)
                            filled_fields.append({'name': inp['name'], 'value': '[first_option]'})
                        # checkbox/radio인 경우
                        ...
                        print(f"      ✓ {inp['name']} = {test_value}")

                    except Exception as fill_error:
                        print(f"      ✗ {inp['name']} 입력 실패: {str(fill_error)[:50]}")
                ...
                if filled_fields:
                    try:
                        ...
                        for selector in submit_selectors:
                            try:
                                if self.page.locator(selector).count() > 0:
                                    ...
                                    self.page.wait_for_timeout(1500)  # 응답 대기
                                    submitted = True
                                    break
                            except:
                                continue
                        ...
                        if submitted:
                            # 최소한의 응답 정보만 캡처 (간단하게)
                            response_info = {
                                'status': 200,  # HTTP 상태 코드 (성공)
                                'url': self.page.url  # 최종 URL (리다이렉트 확인용)
                            }
                            ...
                            print(f"      ✓ 제출 완료! (최종 URL: {response_info['url']})")
                        else:
                            print(f"      ✗ 제출 버튼을 찾을 수 없음")
                            results['errors'].append(f"Submit button not found for form {form_idx}")

                        # 원래 페이지로 복원
                        if self.page.url != original_url:
                            print(f"      [RESTORE] 원래 페이지로 복원 중...")
                            self.page.goto(original_url, wait_until='networkidle')

                    except Exception as submit_error:
                        print(f"      ✗ 제출 실패: {str(submit_error)[:50]}")
                        results['errors'].append(f"Form submit error: {str(submit_error)[:50]}")
                        # 복원 시도
                        try:
                            if self.page.url != original_url:
                                self.page.goto(original_url, wait_until='networkidle')
    ...
    def trigger_interactive_elements(self, max_clicks=10, skip_forms=False):
        """
        발견한 모든 인터랙티브 요소를 자동으로 클릭하여 숨겨진 AJAX 요청 발견

        Args:
            max_clicks: 최대 클릭할 요소 개수
            skip_forms: True면 폼 제출은 건너뜀 (중복 방지)

        Returns:
            dict: 클릭 결과 및 발견된 네트워크 요청 정보
        """
        print(f"[+] 인터랙티브 요소 자동 트리거 시작 (최대 {max_clicks}개, 폼 제외: {skip_forms})...")
        ...
        for i, button in enumerate(buttons):
            ...
            try:
                ...
                # skip_forms가 True면 submit 버튼 건너뛰기
                if skip_forms and button_type == 'submit':
                    print(f"    [SKIP] Submit 버튼 (폼 중복 방지): '{safe_text(button_text)}'")
                    results['skipped_elements'].append({
                        'type': 'button',
                        'text': button_text,
                        'reason': 'skip_forms_enabled'
                    })
                    continue
                ...
                print(f"    [CLICK] 클릭: '{safe_text(button_text)}' (#{i+1})")
                ...
                if self.page.url != original_url:
                    print(f"    [RESTORE]  페이지 변경 감지, 복원 중...")
                    self.page.goto(original_url, wait_until='networkidle')
        ...
        for link_info in js_links:
            ...
            try:
                ...
                if any(keyword in link_text or keyword in link_id
                       for keyword in dangerous_keywords):
                    print(f"    [SKIP]  스킵 (위험): '{link_info['text']}'")
                    results['skipped_elements'].append({
                        'type': 'js_link',
                        'text': link_info['text'],
                        'reason': 'dangerous_keyword'
        ...
        print(f"  [OK] 총 {click_count}개 요소 트리거 완료")
        print(f"  [SKIP]  {len(results['skipped_elements'])}개 스킵")
        print(f"  [ERROR] {len(results['errors'])}개 에러")
...
class SecurityAnalyzer:
    ...
    def analyze_page_security(self, url, response_headers=None):
        """
        페이지의 보안 설정 분석

        Args:
            url: 분석할 URL
            response_headers: HTTP 응답 헤더 (선택)
        ...
        if security_info['csp']:
            ...
            security_info['csp_parsed'] = self.parse_csp(security_info['csp'])
            security_info['csp_issues'] = self.analyze_csp_weaknesses(security_info['csp_parsed'])

            # 취약점 출력
            if security_info['csp_issues']:
                for issue in security_info['csp_issues']:
                    print(f"    🚨 {issue}")
            else:
                print(f"    ✅ CSP 설정 양호")
        else:
            print(f"  → [CSP 없음] ⚠️ XSS 공격에 취약할 수 있음")
        ...
        if response_headers:
            ...
            if missing_headers:
                print(f"  → [보안 헤더 누락]: {', '.join(missing_headers)}")
            ...
            for cookie in all_cookies:
                ...
                if self._is_session_cookie(cookie['name']):
                    if not cookie.get('httpOnly', False):
                        ...
                        print(f"    🚨 {cookie['name']}: HttpOnly ❌ (JS로 탈취 가능!)")
                    else:
                        print(f"    🛡️  {cookie['name']}: HttpOnly ✅ (보호됨)")

                    if not cookie.get('secure', False):
                        print(f"    ⚠️  {cookie['name']}: Secure ❌ (HTTP 전송 가능)")

                    if cookie.get('sameSite', 'None') == 'None':
                        print(f"    ⚠️  {cookie['name']}: SameSite=None (CSRF 취약)")

        except Exception as e:
            print(f"  [!] 쿠키 분석 실패: {e}")

        # 보고서 저장
        self.security_reports.append(security_info)
    ...
    def analyze_csp_weaknesses(self, csp_parsed):
        """
        CSP 취약점 분석

        Args:
            csp_parsed: 파싱된 CSP 정책
        ...
        if "'unsafe-inline'" in script_src:
            issues.append("script-src에 'unsafe-inline' 허용 → 인라인 스크립트 실행 가능 (XSS 위험)")

        if "'unsafe-eval'" in script_src:
            issues.append("script-src에 'unsafe-eval' 허용 → eval() 사용 가능 (코드 인젝션 위험)")

        if '*' in script_src:
            issues.append("script-src에 와일드카드(*) 허용 → 모든 도메인 스크립트 허용 (매우 위험)")

        if 'data:' in script_src:
            issues.append("script-src에 data: URI 허용 → base64 인코딩 스크립트 실행 가능")
    ...
    def _is_session_cookie(self, cookie_name):
        ...
        Returns:
            bool: 세션 쿠키 여부
        """
        keywords = ['session', 'sess', 'auth', 'token', 'login', 'sid', 'phpsessid', 'jsessionid', 'asp.net_sessionid']
        return any(k in cookie_name.lower() for k in keywords)

    def get_all_reports(self):
        """모든 보안 분석 보고서 반환"""
        return self.security_reports

    def get_summary(self):
        """보안 분석 요약 반환"""
        total_pages = len(self.security_reports)
        pages_with_csp = sum(1 for r in self.security_reports if r['csp'])
        total_vulnerable_cookies = sum(len(r['vulnerable_cookies']) for r in self.security_reports)

        return {
            'total_pages_analyzed': total_pages,
            'pages_with_csp': pages_with_csp,
            'pages_without_csp': total_pages - pages_with_csp,
            'total_vulnerable_cookies': total_vulnerable_cookies
        }
    ...
    def show_summary(self):
        ...
        print("보안 분석 요약")
        print("="*60)
        print(f"분석한 페이지: {summary['total_pages_analyzed']}개")
        print(f"CSP 있는 페이지: {summary['pages_with_csp']}개")
        print(f"CSP 없는 페이지: {summary['pages_without_csp']}개")
        print(f"취약한 쿠키: {summary['total_vulnerable_cookies']}개")
        ...
        for report in self.security_reports:
            if report['csp_issues'] or report['vulnerable_cookies']:
                print(f"\n  📄 {report['url']}")
                for issue in report['csp_issues'][:3]:
                    print(f"    - {issue}")
                for cookie in report['vulnerable_cookies']:
...
class AutomatedDynamicCrawler:
    ...
    @staticmethod
    def infer_parameter_value(param_name, param_type, placeholder=None, current_value=None):
        """
        ...
        # 4. submit/button 기본값
        if param_type == 'submit' or param_type == 'button':
            return 'Submit'

        # 5. placeholder를 마지막 fallback으로 사용 (다른 방법이 없을 때만)
        if placeholder:
            return placeholder
    ...
    def start(self):
        ...
        # 네트워크 캡처 시작
        self.network_capturer = NetworkTrafficCapturer(
        ...
        self.security_analyzer = SecurityAnalyzer(
            self.browser_controller.page,
            self.browser_controller.context
        )

        print(f"[+] 브라우저, 네트워크 캡처, 보안 분석 준비 완료\n")
    ...
    def crawl(self, url=None, current_depth=0):
        ...
        try:
            ...
            # 1-1. 보안 분석 (CSP + HttpOnly 확인)
            response_headers = self.network_capturer.get_page_response_headers()
            security_report = self.security_analyzer.analyze_page_security(url, response_headers)
            ...
            # 🆕 폼 자동 제출 (추론된 값으로 실제 제출하여 네트워크 요청 캡처)
            print(f"\n[+] 폼 자동 제출로 실제 API 엔드포인트 확인...")
            form_submit_results = self.content_analyzer.submit_forms_with_test_data(
                infer_value_func=self.infer_parameter_value
            )

            # 🆕 인터랙티브 요소 자동 클릭 (숨겨진 AJAX 요청 발견)
            # skip_forms=True로 폼 중복 제출 방지
            print(f"\n[+] 버튼/드롭다운 자동 클릭으로 숨겨진 API 탐색...")
            click_results = self.content_analyzer.trigger_interactive_elements(max_clicks=15, skip_forms=True)
            print(f"    클릭한 요소: {len(click_results['clicked_elements'])}개")
            print(f"    스킵한 요소: {len(click_results['skipped_elements'])}개")

            # 인터랙션 결과 저장 (페이지 URL과 함께)
            interaction_data = {
                'page_url': url,
                'form_submissions': form_submit_results['submitted_forms'],
                'clicked_elements': click_results['clicked_elements'],
                'skipped_elements': click_results['skipped_elements'],
                'errors': click_results['errors'] + form_submit_results['errors']
            }

            if (form_submit_results['submitted_forms'] or
                click_results['clicked_elements'] or
                click_results['skipped_elements']):
                self.all_interaction_results.append(interaction_data)
    ...
    def _merge_data(self, url, links, forms):
        ...
        for form in forms:
            ...
            for inp in form['inputs']:
                if inp['name']:
                    # 파라미터 값 자동 추론 (placeholder 활용)
                    inferred_value = self.infer_parameter_value(
                        param_name=inp['name'],
                        param_type=inp['type'],
                        placeholder=inp.get('placeholder'),
                    ...
                    param = {
                        'name': inp['name'],
                        'type': 'body' if form['method'].upper() == 'POST' else 'query',
                        'input_type': inp['type'],  # HTML input type (text, password 등)
                        'location': 'body' if form['method'].upper() == 'POST' else 'query',
                        'example_value': inferred_value,  # 자동 추론된 값 사용
    ...
    def export_to_json(self, filename="integrated_crawl_results.json"):
        ...
        # 보안 분석 통계
        security_summary = self.security_analyzer.get_summary() if self.security_analyzer else {}
        ...
        data = {
            ...
            'statistics': {
                ...
                'total_submitted_forms': sum(len(r.get('form_submissions', [])) for r in self.all_interaction_results),
            ...
            'interaction_results': self.all_interaction_results,
            'security_analysis': self.security_analyzer.get_all_reports() if self.security_analyzer else [],
            'network_capture': {
        ...
        print(f"    - 제출한 폼: {sum(len(r.get('form_submissions', [])) for r in self.all_interaction_results)}개")
        ...
        # 보안 분석 결과 출력
        if security_summary:
            print(f"    - 보안 분석: {security_summary['total_pages_analyzed']}개 페이지")
            print(f"      • CSP 있음: {security_summary['pages_with_csp']}개")
            print(f"      • CSP 없음: {security_summary['pages_without_csp']}개")
            print(f"      • 취약한 쿠키: {security_summary['total_vulnerable_cookies']}개")
    ...
    def show_summary(self):
        ...
        if self.security_analyzer:
            sec_summary = self.security_analyzer.get_summary()
            print(f"\n보안 분석:")
            print(f"  - 분석한 페이지: {sec_summary['total_pages_analyzed']}개")
            print(f"  - CSP 있는 페이지: {sec_summary['pages_with_csp']}개")
            print(f"  - CSP 없는 페이지: {sec_summary['pages_without_csp']}개")
            print(f"  - 취약한 쿠키: {sec_summary['total_vulnerable_cookies']}개")
        ...
        # 보안 분석 상세 요약
        if self.security_analyzer:
            self.security_analyzer.show_summary()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. 자동 로그인
        print("\n[*] 로그인 중...")
        login_success = crawler.auto_login()
        ...
        # 5. JSON 저장
        crawler.export_to_json("results.json")
